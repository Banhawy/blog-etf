...
const SocialSharingButtons: React.FC<SocialSharingButtonsProps> = ({
    ...
    return (
        <>
            {all && <AllSocialSharingButtons url={url} />}
            {!all && (
                <div className="flex justify-center">
                    {socials.map((social) => (
                        <SocialSharingButton key={social} social={social} url={url} />
                    ))}
                </div>
            )}
        </>
...
const AllSocialSharingButtons: React.FC<SocialSharingButtonsProps> = ({ url }) => {
    return (
        <>

            {/* <!-- Sharingbutton Facebook --> */}
            <a className="resp-sharing-button__link" href="https://facebook.com/sharer/sharer.php?u=http%3A%2F%2Fsharingbuttons.io" target="_blank" rel="noopener" aria-label="Facebook">
            ...
            <a className="resp-sharing-button__link" href="https://twitter.com/intent/tweet/?text=Super%20fast%20and%20easy%20Social%20Media%20Sharing%20Buttons.%20No%20JavaScript.%20No%20tracking.&amp;url=http%3A%2F%2Fsharingbuttons.io" target="_blank" rel="noopener" aria-label="Twitter">
            ...
            <TelegramButton url={url} />
        </>
    )
}

const SocialSharingButton: React.FC<SocialSharingButtonsProps & { social: string }> = ({ social, url }) => {
    return (
        <>
            {social === 'facebook' && (
                <a
            ...
            {social === 'twitter' && (
                <a
                    className="resp-sharing-button__link"
                    href={`https://twitter.com/intent/tweet/?text=Check%20this%20blog%20post%20on%20%23ChatETF%0A%0A${url}`}
                    target="_blank"
                    rel="noopener"
                    aria-label="Twitter"
                >
...
const LinkedInButton = ({ url }) => {
    ...
    return (
        <a
            className="resp-sharing-button__link"
            href={`https://www.linkedin.com/shareArticle?mini=true&url=${encodedUrl}&title=${title}&source=${source}`}
            target="_blank"
            rel="noopener"
            aria-label="LinkedIn"
        >
...
const RedditButton = ({ url }) => {
    ...
    return (
        <a
            className="resp-sharing-button__link"
            href={`https://reddit.com/submit?url=${encodedUrl}&title=${title}`}
            target="_blank"
            rel="noopener"
            aria-label="Reddit"
        >
...
const WhatsAppButton = ({ url }) => {
    ...
    return (
        <a
            className="resp-sharing-button__link"
            href={`whatsapp://send?text=${encodedUrl}`}
            target="_blank"
            rel="noopener"
            aria-label="WhatsApp">
...
const TelegramButton = ({ url }) => {
    ...
    return (
        <a
            className="resp-sharing-button__link"
            href={`https://telegram.me/share/url?text=${encodedUrl}}`}
            target="_blank"
            rel="noopener"
            aria-label="Telegram"
        >
            <div className="resp-sharing-button resp-sharing-button--telegram resp-sharing-button--medium"><div aria-hidden="true" className="resp-sharing-button__icon resp-sharing-button__icon--solid">
